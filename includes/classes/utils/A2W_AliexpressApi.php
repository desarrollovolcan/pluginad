        ...
        public function load_product_data($product_id, $session, $params = array()){
            ...
            if ($data_v1['state'] === 'error' && $data_v2['state'] === 'error'){
               if ($data_v1['state'] === 'error'){
                ...
                $other_currencies_exchange_rates = array();
                if ($currency_exchange_rate){
                    ...
                    foreach ( $data_v1['product']['sku_products']['variations'] as &$var) {
                        if ($var['currency'] === 'USD'){
                            $selected_exchange_rate = $currency_exchange_rate;
                        } else {
                            //sometimes API V1 returns prices in other currency
                            $other_currency = $var['currency'];
                            if (!isset($other_currencies_exchange_rates[$other_currency])){
                                $currencyApi = new A2W_CurrencyApi();
                                $currency_api_result = $currencyApi->get_conversion_rate($current_currency, $other_currency);
                                if ($currency_api_result['state'] !== 'error'){
                                    $other_currencies_exchange_rates[$other_currency] = $currency_api_result['rate'];
                                } else {
                                    return $currency_api_result;    
                                }
                            }
                            $selected_exchange_rate = $other_currencies_exchange_rates[$other_currency]; 
                        }  
                        ...
                        if (isset($body['aliexpress_ds_product_get_response']) && !empty($body['aliexpress_ds_product_get_response']['result'])){
                            ...
                            if (!isset($body['error_response'])){
                                if (!$api_us_id_fix){
                                    ...
                                    $result = A2W_ResultBuilder::buildError(__('No such a product available',  'ali2woo'));   
                                }
                            } else {
              ...
              if (is_wp_error($request)) {
                  ...
                  if (intval($request['response']['code']) == 200) {
                    ...
                    if (!isset($body['error_response'])){
                        ...
                        $result = A2W_ResultBuilder::buildError($body['error_response']['msg'] . ' - ' . $body['error_response']['sub_msg']);
                    }
